...
工作流，包含 dose grid、centering/position/layer/random-subspace 對照、anonymous
與 dial probes，以及 G1a/G1b/G2/G3/G4 評估。Package：
`llm_bias/selective_intervention/`；輸出：
`artifacts/<model-slug>/selective-intervention/runs/<run-id>/`。
協議見 [V1 protocol](selective-intervention/details/proposal-v1.md)，formal 結果見
...
| `entity_cell_factual_recall_preflight.py` | Factual recall preflight（proposed，非 frozen）：重用 E1 V2 frozen factual-cloze frames，在 clean baseline 捕捉各 entity 的模型自身完成式（top-5 與 5-token greedy）與 Anonymous 對照（每 frame 一次），判斷是否存在 entity-specific 事實訊號；`--entities TICKER:Name...`（`|` 接多 surface variants）可覆蓋內建 entity 集，`--frames ID:template...` 可覆蓋內建 frame 集（E5 preflight 以此測試新事實類別句框） | `--output` 指定的 compact JSON（per-frame top-k、greedy 續寫、metadata） | [Entity cell localization README「Proposed: factual recall preflight」](entity-cell-localization/details/README.md) / [E5 proposal](entity-cell-localization/details/proposal-e5.md) |
...
| `entity_cell_fact_coverage_probe.py` | E5 fact coverage profile probe（proposed，**待實作**）：對 4 個 frozen V3 entity cells 在 18 個 (entity, frame) pairs（HQ 雙句框、代號、年份、交易所、競爭對手）做 clean / target / matched-random / wrong-entity 四條件的輸出層 gold joint log-prob collapse 量測，產出 per-cell 事實覆蓋輪廓；重用 frozen `fact_amnesia` 計分與 `mlp_cells` 干預/配對代碼；gold 綁定 E5 preflight 記錄（fail-closed）、錨點對照 frozen V3 | `--output` 指定的 compact JSON（per-(entity,frame,condition) collapse + 覆蓋矩陣 + provenance） | [E5 proposal](entity-cell-localization/details/proposal-e5.md) |

...
| Cross-sector context overriding    | B V1    | **Discovery complete；calibration `success=false`**。L16 Context effect 與 Context–Header contrast 通過 effect/statistical gates，但 same-sector peer specificity gate 未通過；`test_authorized=false`，見 [confirmation report](report-confirmation.md) |

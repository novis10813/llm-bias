...
**狀態：已收線（Phase A–F）。** 模型為 Qwen3.5-4B（bf16，hybrid Gated DeltaNet 架構，32 層）；2026-09-13 收線，Phase E1 通過、Phase E2b/F1 未通過（雙通道表示被否決），最終採納 L15 instruction span 的 $k=8$ 殘差子空間描述收線。

**一句話發現：** 實體訊號由 L15 指令區間的 8 維殘差子空間（$k=8$ 承載 98.3% 充分性）主體傳遞，而不經由實體 token 亦不經由單一 Dial 瓶頸；「$v_1$ + Dial」雙通道在下游讀出飽和匯流（加法比值 0.733 < 0.85，Gate F1 fail）且 $v_1$ 脫離上下文無獨立立場 loading，雙通道緊湊表示被否決，以 $k=8$ 殘差子空間收線。
...
1. **樣本與方向母體限制**：結論基於 16 家標竿企業與 8 個極端對比方向（TOP = {NSC, BLK}，BOTTOM = {IT, BDX}），未外推至全域企業或中性對比方向。
2. **非電路級完全拆解**：dual-hook 屬狀態介入，證實兩通道下游飽和，不等於還原內部計算圖突觸權重。

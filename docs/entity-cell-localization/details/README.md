...
**收線報告：** [report-line-closing.md](../report.md)——整條線的敘事入口
...
- E2/E3 battery (JNJ/JPM): `entity-cell-e2-hfm2-v1` (complete, attribution + patching + analyze；readout 未執行)；`entity-cell-e2-hfm2-readout-v1` (complete, 2026-09-07, 同條件重跑並補 e2-readout，pinned HF lens 條件，見 [battery 報告 §2a](report-jnj-jpm-battery.md))；伴隨 compact outputs `flip_probe_jnj_neutral.json`、`flip_probe_jpm_neutral.json`、`flip_probe_jnj_band.json`、`e3_records_jnj.json`、`inputs_jnj_band.jsonl`（placeholder 證據）

...
The [research proposal](entity-bias-research-proposal.md) defines the questions and claim limits; the [research directory](../README.md) links all active experiment protocols and results. This revision changes program scope, not historical experiment gates or authorizations. The frozen exploratory [financial-judgment workflow](../archive/financial-soundness-localization/proposal.md) is archive material, not active evidence.
...
| [Entity Cell](../entity-cell-localization/report.md) | Main line closed; V3 four factual cells confirmed in calibration/hold-out; interventions affect tested fact subsets and cross-company checks find shared channels; E4 probe completed but proposed. | Supports partial entity representations rather than one-neuron complete entity storage. Factual memory certification and decision probes are separate evidence, not an entity-control success. |
| [Investment-dial](../investment-dial/report.md) | Closed; V2 calibration pass, B reevaluation, not fresh held-out validation. | A tested neuron can affect a global decision score, but this is global stance modulation rather than evidence of an entity-specific decision unit; independent method replication, not entity-specific control. |
| [Balanced Evidence Gap](../balanced-evidence-gap/report.md) | Phase 1 behaviour confirmed; Phase 2 discovery completed; Phase 3 causal gate fails, 0/3 confirmed. | Entity gap and layer-band evidence; selected late neurons do not pass causal validation, but the null is limited to selected coordinates and interventions and does not exclude a non-orthogonal downstream projection mechanism. |
| [Entity-to-Dial](../entity-to-dial/report.md) | Phase A–F completed and closed; L15 k=8 effect ratio 0.983 in tested directions; F1 additivity fails; held-out transfer V1 (200-company, 2026-09-22) completed with frozen V8 median recovery 0.6759 sector-within / 0.6596 cross-sector, both below the 0.8 target and both above random-8D controls. | Residual-state transfer is a better description than a single dial, entity token or additive two-channel account; high-recovery held-out subspace generalisation is not supported (V1), and this is not complete circuit recovery or selective gap reduction. |

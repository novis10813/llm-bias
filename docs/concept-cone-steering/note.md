...
#### 命題 1：抗飽和性與單調平滑縮放（Anti-Saturation & Monotonic Scaling）
在 MO 上比較 1D ($b_1$)、4D Cone 質心、8D Cone 質心隨 $\alpha \in [0.0, 8.0]$ 的推進軌跡：
- **1D ($b_1$)**：Margin 由 −1.967 單調升至 +2.511，$\alpha=4.0$ 跨過翻轉門檻（+1.132, `buy`）。
- **4D Cone 質心**：Margin 由 −1.967 平滑、單調攀升至 +2.082，在 **$\alpha=5.0$ 穩定翻轉為 `buy`（+1.287）**。
- **8D Cone 質心**：由 −1.967 穩定爬升至 +0.228，全幅無任何舊基底的反折或提早飽和現象。
- **結論：** 徹底消除了舊 16 家基底的「提早飽和與反折」缺陷，驗證了多維圓錐的平滑單調縮放性。

#### 命題 2：生成理由的語義互補性（Semantic Complementarity within Cone）
在 4 維圓錐 $\mathcal{R}_4$ 內部取樣 4 條不同偏向的正交邊緣射線（Ray A–D）與平衡質心（Ray E），於 $\alpha=5.0$ 檢視生成的決策理由：
- **Ray A（重偏 $b_1$ 主軸）**：Margin **+2.155（`buy`）**，理由聚焦於「14% 營收成長壓過利潤率與財測下修」。
- **Ray B（重偏 $b_2$ 互補軸）**：Margin **−1.287（`sell`）**，強烈錨定於「毛利縮減 300 bps 且財測下修 6% 的利空」。
- **Ray C（重偏 $b_3$ 互補軸）**：Margin **−0.056（`sell`）**，精確將決策推至多空勢均力敵的零點刀鋒邊界。
- **Ray E（平衡 4D 質心）**：Margin **+1.287（`buy`）**，平滑調和各軸，穩健輸出 `buy`。
- **結論：** 證實圓錐內部的不同正交維度調控的是財務證據的不同考量面向，並非單一標量的冗餘複製。

#### 命題 3：跨產業、跨公司通用性（Cross-Company Generalization）
以同一套 4D Concept Cone 質心，在 5 家橫跨不同行業的極端 Sell 公司上測試泛化翻轉（多空平衡提示詞）：

| 受測公司 | 產業分類 | Clean 基準 ($T=0$) | $\alpha=3.0$ | $\alpha=4.0$ | $\alpha=5.0$ | $\alpha=6.0$ | 最終翻轉點 ($T=0$) |
|---|---|---:|---:|---:|---:|---:|---|
| **MO (Altria)** | 必需消費 (菸草) | −1.967 (`sell`) | −0.049 | +0.704 | **+1.287 (`buy`)** | +1.719 (`buy`) | **$\alpha=5.0$ 翻轉** |
| **CNC (Centene)** | 醫療保健 (保險) | −2.059 (`sell`) | −0.484 | +0.275 | +0.816 | **+1.384 (`buy`)** | **$\alpha=6.0$ 翻轉** |
| **FOXA (Fox)** | 通訊服務 (傳媒) | −1.867 (`sell`) | −0.047 | +0.656 | **+1.347 (`buy`)** | +1.807 (`buy`) | **$\alpha=5.0$ 翻轉** |
| **TSN (Tyson Foods)** | 必需消費 (食品) | −1.730 (`sell`) | +0.267 | +1.025 | **+1.523 (`buy`)** | +1.940 (`buy`) | **$\alpha=5.0$ 翻轉** |
| **BAX (Baxter Intl)** | 醫療設備 (器械) | −1.676 (`sell`) | +0.185 | +0.880 | **+1.470 (`buy`)** | +1.871 (`buy`) | **$\alpha=5.0$ 翻轉** |

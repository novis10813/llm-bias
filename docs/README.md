...
| Evidence-insensitivity：Phase 1 **completed**（2026-09-16）；S&P 500 全人口（503 家）共用 company-agnostic 證據下的證據不敏感性行為篩選與分組（50 responsive / 453 fixed-sell、503/503 零證據 sell、order-swap recency 效應；cross-model 診斷確認現象為模型特性）；Phase 2（capture-layer 組間對比）completed（雙模型：狀態不承載行為分組，offset/gain 皆 null）；Phase 3（上游因果定位，雙模型干預）completed（2026-09-17：span 位置全域 null、極性 contrast 在 final-position 狀態、S-vs-R 皆 readout-mediated 且組差方向跨模型相反、0 flip）。 | [proposal](evidence-insensitivity/proposal.md) / [Phase 1 報告](evidence-insensitivity/details/report-phase1.md) / [Phase 2 報告](evidence-insensitivity/details/report-phase2.md) / [Phase 3 報告](evidence-insensitivity/details/report-phase3.md) |

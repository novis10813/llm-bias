"""Bounded axis-decomposition operator: sweep additive pushes along the
L15 instruction-span content axes (entity-contrast PCs, stance, shared
mean-state, randoms) and re-score the buy/sell margin.

Development only, no gates.  Reuses the frozen prepared prompts and materials
of the stance characterization run, so the stance axis is refit from exactly
...
def main() -> None:
    ...
    run = run_axis_decomposition(
        ...
        doses=args.doses, n_pcs=args.n_pcs, n_random=args.n_random, random_seed=args.seed,
        provenance={
            "mode": "model_smoke",
    ...
    print(f"Completed axis decomposition: {run}", flush=True)
